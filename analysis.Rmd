...
model w/ online vs in-person, looking vs non-looking, moderated vs unmoderated, and mean age as moderators
```{r}
d_calc$d_calc
full_mod <- rma.mv(yi = d_calc, 
                   V = d_var_calc,
                   mods = ~ as.factor(expt_condition) 
...
Forest plot organized by response mode (looking vs non-looking)

```{r}
d_diff <- d_calc |>
  group_by(paper_ID, experiment_ID, response_mode) %>%
  summarise(d_diff = mean(d_calc[expt_condition == "online"], na.rm=TRUE) - 
              mean(d_calc[expt_condition == "in-person"], na.rm=TRUE)) 
...
ggplot(d_diff, aes(x = short_cite, y = d_diff, color = response_mode)) + 
  geom_point() + 
  coord_flip() + 
  scale_color_manual(values = wes_palette("Chevalier1", n = 2)) +
  geom_hline(yintercept = 0, lty = 2) + 
  ylab("SMD difference (online - in-person)") + 
  xlab("")
...
Forest plot organized by method (moderated vs unmoderated)
...
d_diff <- d_calc |>
  ...
  summarise(d_diff = mean(d_calc[expt_condition == "online"], na.rm=TRUE) - 
              mean(d_calc[expt_condition == "in-person"], na.rm=TRUE),
            method = method[expt_condition == "online"]) 

d_diff <- d_diff |>
  left_join(d_calc |> select(paper_ID, experiment_ID, short_cite))

ggplot(d_diff, aes(x = short_cite, y = d_diff, color = method)) + 
  geom_point() + 
  coord_flip() + 
  scale_color_manual(values = wes_palette("Chevalier1", n = 2)) +
  geom_hline(yintercept = 0, lty = 2) + 
...
Forest plot organized by mean age
...
d_diff
ggplot(d_diff, aes(x = short_cite, y = d_diff, color = mean_age)) + 
  geom_point() + 
  coord_flip() + 
  geom_hline(yintercept = 0, lty = 2) + 
  ylab("SMD difference (online - in-person)") + 
