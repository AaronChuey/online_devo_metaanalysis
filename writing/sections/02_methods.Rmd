...
foo$boxtext[4] <- "Identification of studies via forward citation search, Frontiers special issue, and listservs"
...
foo$n[7] <- d |> filter(!Source %in% c( "Lookit1", 
                    "Lookit2", "Rhodes", "Sheskin&Keil", "Zaadnoordijk&Cusack")) |> nrow()
...
PRISMA_data(foo) |> PRISMA_flowdiagram(previous=F, other=F, detail_databases = T, detail_registers = T, fontsize=10,side_boxes=F)
```
