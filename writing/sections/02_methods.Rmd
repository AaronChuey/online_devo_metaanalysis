...
d |> filter(expt_condition=="in-person") |> select(main_title, `comparison-title`, short_cite, comparison_cite) |> unique() |> left_join(studies) |> 
  ...
  mutate(short_cite=str_c("@", short_cite), comparison_cite=ifelse(is.na(comparison_cite),"",str_c("and @",comparison_cite))) |> 
  mutate(`Papers used in this meta-analysis`=str_c(short_cite," ",comparison_cite)) |> 
  
  select(`Papers used in this meta-analysis`,`Num pairs`=pairs, `Method`=method, `Moderated?`=moderation, `Average age (mos)`=avg_age) |> 
   unique() |> 
