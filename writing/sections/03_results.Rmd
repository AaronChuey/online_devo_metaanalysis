...
```{r e_coeffs, fig.pos="ht"}

# library(magick)
combined_mod <- rma.mv(yi = d_calc, 
                        V = d_var_calc, 
                        mods = ~ as.factor(expt_condition) 
                        * as.factor(method) * as.factor(response_mode),
                        random = ~1 | as.factor(experiment_ID) + 
                          as.factor(same_infant), 
                        slab = short_cite, 
                        data = d_calc)
...
aggregated <- aggregate(d_calc$d_var_calc, list(d_calc$same_infant), mean) |> 
  rename(same_infant = Group.1,
         mean_smd = x)
combined <- merge(d_calc, aggregated)
...
grouped_calc |>
  knitr::kable(#caption = "Mean SMD by study modality, data-collection method, and type of dependent measure.", 
    digits = 3) |> 
  kable_styling(latex_options = "hold_position")
  
```

Additionally, we examined which combinations of methods and measures tended to yield the strongest and weakest effect sizes relative to their in-person counterparts. Although our model is likely too underpowered to detect a significant interaction between study modality, online method, and dependent measure, unmoderated studies with looking measures yielded the noticeably weakest effect sizes compared to their in-person counterparts, an average difference of `r grouped_calc$SMD[1] - grouped_calc$SMD[5]` SMD (see Table \@ref(tab:e_coeffs)).
...
We also conducted an exploratory analysis of potential publication bias. It was unclear how we might expect publication biases to manifest themselves, given that there is some possibility of notoriety for either showing *or* failing to show differences between online and in-person testing.
