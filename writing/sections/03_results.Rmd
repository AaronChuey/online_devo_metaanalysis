...
```{r forest, fig.env="figure", fig.pos = "h", fig.align = "center",out.width="100%", fig.width=10, fig.height=5, fig.cap = "Forest plots of studies, sorted by difference in SMD. Each dot is the difference between and in-person measure and a corresponding online measure. Each row is one study (paper or pair of papers)." }
...
Because our meta-analysis averaged across effects from very different paradigms (which could yield different effect sizes independent of the effect of testing modality), we expected substantial heterogeneity. Consistent with that expectation, all tests for residual heterogeneity were highly significant (all $p$s $< .0001$). Values of $\tau^2$ for the models were `r round(main_mod$tau2,2)` (primary model), `r round(looking_mod$tau2,2)` (looking-time model), `r round(age_mod$tau2,2)` (age model), and `r round(moderated_mod$tau2,2)` (moderated vs. unmoderated model), respectively, confirming the impression that these moderators did not reduce heterogeneity.
...
TOSTtwo(m1=mean(d_inperson$mean_d),m2=mean(d_online$mean_d),sd1=sd(d_inperson$mean_d),sd2=sd(d_online$mean_d),n1=nrow(d_inperson),n2=nrow(d_online),low_eqbound_d=0, high_eqbound_d=threshold, alpha = 0.05, var.equal=FALSE)
```

In addition to our multi-level meta-analysis, we conducted an equivalence test to determine whether the difference in effect size between online and in-person modalities meaningfully differed... 
...
combined <- aggregate(d_calc$d_calc, 
                      list(d_calc$same_infant), 
                      mean) |> 
  rename(same_infant = Group.1,
         mean_smd = x) |> 
  merge(d_calc)

grouped_calc <- combined |>
  group_by(as.factor(expt_condition), as.factor(method), as.factor(response_mode)) |> 
  summarise(SMD = mean(mean_smd))
...
We additionally conducted an exploratory analysis of potential publication bias. It was unclear *a priori* how we might expect publication biases to manifest themselves, given that there is some possibility of notoriety for either showing *or* failing to show differences between online and in-person testing. In either case our hypothesized selection process operated on the *differences* in effect sizes between each online and in-lab pair of samples.

For each online and in-person pair on the same study, we calculated a standard mean difference in effect size between the two studies as well as the variance of this difference. The resulting funnel plot is shown in Figure \@ref(fig:funnel). According to Egger's regression test for funnel plot asymmetry, this plot is asymmetric (p=`r papaja::printp(funtest$pval)`) and the limit estimate of the effect as standard error goes to zero is `r funtest$est |> round(2)` [`r funtest$ci.lb |> round(2)`, `r funtest$ci.ub |> round(2)`]. This analysis suggests the possibility of publication bias favoring negative comparisons between online and in-lab modalities, signaling that perhaps the true effect of online testing is zero or perhaps even positive. We interpret this conclusion with caution, however, noting the large width of the estimated CI and the relatively low power of Egger's test [@sterne2000]. 
