...
For each online and in-person pair on the same study, we calculated a standard mean difference in effect size between the two studies as well as the variance of this difference. The resulting funnel plot is shown in Figure \@ref(fig:funnel). According to Egger's regression test for funnel plot asymmetry, this plot is asymmetric (p=`r papaja::printp(funtest$pval)`) and the limit estimate of the effect as standard error goes to zero is `r funtest$est |> round(2)` [`r funtest$ci.lb |> round(2)`, `r funtest$ci.ub |> round(2)`]. This analysis suggests the possibility of publication bias favoring negative comparisons between online and in-lab modalities, signalling that perhaps the true effect of online testing is zero or perhaps even positive. We interpret this conclusion with caution, however, noting the large width of the estimated CI and the relatively low power of Egger's test [@sterne2000]. 
