...
Because our meta-analysis averaged across effects from very different paradigms (which could yield different effect sizes independent of the effect of testing modality), we expected substantial heterogeneity. Consistent with that expectation, all tests for residual heterogeneity were highly significant (all \(p\)s \(< .0001\)). Values of \(\tau^2\) for the models were 0.23 (primary model), 0.23 (looking-time model), 0.23 (age model), and 0.23 (moderated vs.~unmoderated model), respectively, confirming the impression that these moderators did not reduce heterogeneity.
...
\midrule()
\endhead
In-person & Moderated & Looking & 0.699 \\
In-person & Moderated & Verbal & 0.677 \\
Online & Moderated & Looking & 0.597 \\
Online & Moderated & Verbal & 0.511 \\
Online & Unmoderated & Looking & 0.177 \\
Online & Unmoderated & Verbal & 0.570 \\
\bottomrule()
\end{longtable}
...
We additionally conducted an exploratory analysis of potential publication bias. It was unclear \emph{a priori} how we might expect publication biases to manifest themselves, given that there is some possibility of notoriety for either showing \emph{or} failing to show differences between online and in-person testing. In either case our hypothesized selection process operated on the \emph{differences} in effect sizes between each online and in-lab pair of samples.

For each online and in-person pair on the same study, we calculated a standard mean difference in effect size between the two studies as well as the variance of this difference. The resulting funnel plot is shown in Figure \ref{fig:funnel}. According to Egger's regression test for funnel plot asymmetry, this plot is asymmetric (p=.005) and the limit estimate of the effect as standard error goes to zero is 0.37 {[}0.01, 0.72{]}. This analysis suggests the possibility of publication bias favoring negative comparisons between online and in-lab modalities, signalling that perhaps the true effect of online testing is zero or perhaps even positive. We interpret this conclusion with caution, however, noting the large width of the estimated CI and the relatively low power of Egger's test (Sterne, Gavaghan, \& Egger, 2000).
