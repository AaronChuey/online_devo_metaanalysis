...
\caption{Forest plots of studies. In left and center plots, each row is one study, each dot is one paper and each dot is the difference between an in-person measure and corresponding online measure. On right plot, y-axis is the average age of the children in the two samples being compared. \label{forest}}\label{fig:forest}
\end{figure}

\begin{table}

\caption{\label{tab:coeffs}Table of coefficients for the pre-registered models. The overall model is shown first, followed by the three models with moderators. \label{coeffs}}
\centering
\begin{tabular}[t]{lrlr}
\toprule
Coefficient & Estimate & Conf. Int. & P-value\\
\midrule
\addlinespace[0.3em]
